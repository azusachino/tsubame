//! Simple HTTP/3 GET request client example
...
async fn main() -> Result<()> {
    ...
    config.verify_peer(false); // Skip certificate verification for self-signed certs
    ...
    loop {
        ...
        match conn.recv(&mut buf[..len], recv_info) {
            Ok(len) => log::debug!("Received {} bytes", len),
            Err(e) => {
    ...
    log::info!("Sent HTTP/3 request on stream {}", stream_id);
